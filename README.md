...
Create chapters with the following YAML front matter:
...
1. `published`: If `false`, the chapter will not be rendered and will not be included in the chapter numbering. 
1. `disable_toc`: If `true`, don't render the chapter's headings as a table of contents at the top of the page.
1.  `class`: Optionally specify CSS class(es) to add to the `<div>` wrapping the page.
